...
pub mod Bridge {
    ...
    /// Check if daily bridge limit is exceeded (disabled for now)
    fn check_daily_limit(ref self: ContractState, amount: u256) {
        // Daily limit check disabled - allowing all transactions
    ...
    /// Update daily bridge usage (disabled - no longer tracking daily usage)
    fn update_daily_usage(ref self: ContractState, amount: u256) {
        // Daily usage tracking disabled
        // This function is kept for backward compatibility but does nothing
    }
